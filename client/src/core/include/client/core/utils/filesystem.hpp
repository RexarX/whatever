...
enum class FileError : uint8_t {
  kCouldNotOpen,  ///< Failed to open the file.
  kReadError,     ///< Error occurred while reading the file.
};
 ...
 * @param filepath The path to the file as a string view.
 ...
 */
[[nodiscard]] constexpr std::string_view GetFileExtension(std::string_view path) noexcept {
  const size_t last_dot = path.find_last_of('.');
  return (last_dot != std::string_view::npos) ? path.substr(last_dot) : "";
}

/**
 * @brief Extracts the directory portion from a given path.
 * @param path The full file path.
 * @return The directory portion of the path, or an empty string if none exists.
 */
[[nodiscard]] constexpr std::string_view GetDirectory(std::string_view path) noexcept {
  const size_t last_slash = path.find_last_of("/\\");
  return (last_slash != std::string_view::npos) ? path.substr(0, last_slash) : "";
}
 ...
 */
[[nodiscard]] constexpr std::string_view GetFileNameWithoutExtension(std::string_view path) noexcept {
  const std::string_view filename = GetFileName(path);
  const size_t last_dot = filename.find_last_of('.');
  return (last_dot != std::string_view::npos) ? filename.substr(0, last_dot) : filename;
}
