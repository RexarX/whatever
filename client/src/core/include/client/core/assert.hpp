 ...
 * `LogAssertionFailureViaLogger` in `client::details`.
 ...
 *
 * If the logger isn't available (or calling it throws),
 * this function simply returns so the caller may perform a fallback print-and-abort.
 *
 * @param condition The failed condition as a string
 * @param loc Source location of the assertion
 * @param message Additional message to log
...
#define CLIENT_ASSERT(condition, ...)                                                             \
  do {                                                                                            \
    if constexpr (::client::details::kEnableAssert) {                                             \
      if (CLIENT_EXPECT_FALSE(!(condition))) [[unlikely]] {                                       \
        constexpr auto loc = std::source_location::current();                                     \
        if constexpr (sizeof(#__VA_ARGS__) > 1) {                                                 \
          try {                                                                                   \
            const std::string msg = std::format("" __VA_ARGS__);                                  \
            ::client::details::AssertionFailed(#condition, loc, msg);                             \
          } catch (...) {                                                                         \
            ::client::details::AssertionFailed(#condition, loc, "Formatting error in assertion"); \
          }                                                                                       \
        } else {                                                                                  \
          ::client::details::AssertionFailed(#condition, loc, "");                                \
        }                                                                                         \
        CLIENT_DEBUG_BREAK();                                                                     \
      }                                                                                           \
    }                                                                                             \
...
#else
#define CLIENT_ASSERT(condition, ...) [[maybe_unused]] static constexpr auto CLIENT_ANONYMOUS_VAR(unused_assert) = 0
#endif
 ...
 */
#ifdef CLIENT_ENABLE_ASSERTS
#define CLIENT_INVARIANT(condition, ...)                                                        \
  do {                                                                                          \
    if (CLIENT_EXPECT_FALSE(!(condition))) [[unlikely]] {                                       \
      constexpr auto loc = std::source_location::current();                                     \
      if constexpr (sizeof(#__VA_ARGS__) > 1) {                                                 \
        try {                                                                                   \
          ...
          ::client::details::AssertionFailed(#condition, loc, "Formatting error in invariant"); \
        }                                                                                       \
      } else {                                                                                  \
        ::client::details::AssertionFailed(#condition, loc, "");                                \
      }                                                                                         \
  ...
  do {                                                                  \
    if (CLIENT_EXPECT_FALSE(!(condition))) [[unlikely]] {               \
      ...
      if constexpr (sizeof(#__VA_ARGS__) > 1) {                         \
        try {                                                           \
          const std::string msg = std::format("" __VA_ARGS__);          \
          ::client::details::LogAssertionFailure(#condition, loc, msg); \
        } catch (...) {                                                 \
          ::client::details::LogAssertionFailure(#condition, loc, "");  \
        }                                                               \
      } else {                                                          \
        ::client::details::LogAssertionFailure(#condition, loc, "");    \
      }                                                                 \
    ...
    if (CLIENT_EXPECT_FALSE(!(condition))) [[unlikely]] {                                    \
      ...
      if constexpr (sizeof(#__VA_ARGS__) > 1) {                                              \
        try {                                                                                \
          ...
          ::client::details::AssertionFailed(#condition, loc, "Formatting error in verify"); \
        }                                                                                    \
      } else {                                                                               \
        ::client::details::AssertionFailed(#condition, loc, "");                             \
      }                                                                                      \
 ...
 * @details Similar to CLIENT_VERIFY but allows specifying a custom logger.
 ...
 * @hideinitializer
 */
#define CLIENT_VERIFY_LOGGER(logger_name, condition, ...)                                    \
  do {                                                                                       \
    if (CLIENT_EXPECT_FALSE(!(condition))) [[unlikely]] {                                    \
      constexpr auto loc = std::source_location::current();                                  \
      if constexpr (sizeof(#__VA_ARGS__) > 1) {                                              \
