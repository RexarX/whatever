...
def main() -> int:
    ...
    print_info("Client Code Formatting Tool")
    print_info("=" * 40)
    ...
    if not check_clang_format():
        ...
        print_info("  Debian/Ubuntu: sudo apt install clang-format")
        print_info("  Fedora/RHEL:   sudo dnf install clang-tools-extra")
        print_info("  Arch Linux:    sudo pacman -S clang")
    ...
    extensions = ["cpp", "c", "h", "hpp", "inl", "cc", "cxx"]

    # Define directories to exclude
    exclude_dirs = [
        project_root / "build",
        project_root / "third-party",
        project_root / ".cpm_cache",
    ]
    ...
    if args.check:
        ...
        for file_path in source_files:
            ...
            print_info(f"Formatting: {relative_path}")
