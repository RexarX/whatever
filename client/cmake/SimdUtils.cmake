...
if(DEFINED CLIENT_SIMD_UTILS_LOADED)
    return()
endif()
set(CLIENT_SIMD_UTILS_LOADED TRUE)

include(CheckCXXCompilerFlag)

# Cache variable for SIMD level selection
set(CLIENT_SIMD_LEVEL "native" CACHE STRING "SIMD level: none native sse2 sse3 ssse3 sse4.1 sse4.2 avx avx2 avx512")
set_property(CACHE CLIENT_SIMD_LEVEL PROPERTY STRINGS
    "none" "native" "sse2" "sse3" "ssse3" "sse4.1" "sse4.2" "avx" "avx2" "avx512"
)

# Internal: Map SIMD level to compiler flags
function(client_simd_build_flag_lists LEVEL OUT_GNU_FLAGS OUT_MSVC_FLAGS)
    set(_gnu "")
    set(_msvc "")
...
function(client_simd_filter_supported_flags FLAGS_LIST_VAR OUT_VAR)
    set(_in ${${FLAGS_LIST_VAR}})
    set(_accepted "")

    foreach(_f IN LISTS _in)
        string(MAKE_C_IDENTIFIER "CLIENT_SIMD_${_f}" _var_name)
...
# Usage:
#   client_simd_apply_to_target(TARGET mylib LEVEL avx2)
#   client_simd_apply_to_target(TARGET mylib) # uses CLIENT_SIMD_LEVEL
function(client_simd_apply_to_target)
    cmake_parse_arguments(SIMD "" "TARGET;LEVEL" "" ${ARGN})

    if(NOT SIMD_TARGET)
        message(FATAL_ERROR "client_simd_apply_to_target: TARGET is required")
    endif()

    set(_level "${SIMD_LEVEL}")
    if(NOT SIMD_LEVEL)
        set(_level "${CLIENT_SIMD_LEVEL}")
    endif()
    ...
    # Build flag lists
    client_simd_build_flag_lists("${_level}" _gnu_flags _msvc_flags)

    # Filter to supported flags
    client_simd_filter_supported_flags(_gnu_flags _gnu_accepted)
    client_simd_filter_supported_flags(_msvc_flags _msvc_accepted)
...
# Usage:
#   client_simd_create_interface_libs()
#   target_link_libraries(mytarget PRIVATE simd::avx2)
function(client_simd_create_interface_libs)
    ...
    foreach(_lvl IN LISTS _levels)
        ...
        # Get flags
        client_simd_build_flag_lists("${_lvl}" _g _m)
        client_simd_filter_supported_flags(_g _g_ok)
        client_simd_filter_supported_flags(_m _m_ok)
...
function(client_simd_known_levels OUT_VAR)
    set(_levels "none" "native" "sse2" "sse3" "ssse3" "sse4.1" "sse4.2" "avx" "avx2" "avx512")
